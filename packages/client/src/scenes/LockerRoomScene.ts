...
export class LockerRoomScene {
  ...
  constructor(private ctx: GameContext) {
    ...
    this.preview = this.createPreview(W, H);
    this.uiEl = this.buildUI();
    document.getElementById('ui-layer')!.appendChild(this.uiEl);
  }

  private createPreview(W: number, H: number): Fighter {
    const f = new Fighter({
    ...
    f.container.scale.set(2.5);
    f.container.x = W / 2;
    f.container.y = H * 0.45;
  ...
  private buildUI(): HTMLDivElement {
    ...
    panel.style.cssText = 'pointer-events:all;background:rgba(10,10,30,0.9);border:1px solid rgba(255,215,0,0.3);border-radius:14px;padding:20px;width:min(400px,90vw);display:flex;flex-direction:column;gap:14px;';
    ...
    panel.appendChild(this.makeField('Fighter Name', () => {
      ...
      inp.addEventListener('input', () => { this.charName = inp.value; this.rebuildPreview(); });
      return inp;
    }));
    ...
    panel.appendChild(this.makeField('Outfit Color', () => {
      ...
      OUTFIT_COLORS.forEach(col => {
        ...
        dot.addEventListener('click', () => {
          ...
          wrap.querySelectorAll('button').forEach(b => { (b as HTMLElement).style.border = '2px solid transparent'; });
    ...
    saveBtn.addEventListener('click', () => void this.saveAndBack());
  ...
  private async saveAndBack() {
    ...
    try {
      await fetch(`/player/${player.id}/cosmetics`, {
        ...
        body: JSON.stringify({ hairStyle: player.cosmetics.hairStyle ?? 0, outfitColor: this.outfitColor, auraColor: this.auraColor, characterName: this.charName }),
      });
    } catch { /* offline — cosmetics saved locally */ }
